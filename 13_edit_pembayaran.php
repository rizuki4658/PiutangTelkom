          ...
          <div class="">
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2><i class="fa fa-credit-card"></i> <small>EDIT PEMBAYARAN CICILAN (<?php echo $_GET['cst']." No. SND ".$_GET['snd']; ?>)</small></h2>
                  ...
                  <div class="x_content">
                    ...
                    <form class="demo-form" name="formcicil" method="post" action="./6_detil_piutang/edit_pembayaran.php">
                            ...
                            <?php
                              $kode_p=$_GET['kode'];
                              $kode_pJ=$_GET['kode']."BD";
                              $query_detil="SELECT * FROM detail_piutang WHERE kode_p='$kode_p'";
                              $query_jurnal="SELECT * FROM jurnal WHERE kode_p='$kode_pJ'";
                              $execute_detil=mysqli_query($link, $query_detil);
                              $execute_jurnal=mysqli_query($link, $query_jurnal);
                              if (mysqli_num_rows($execute_detil)>0 && mysqli_num_rows($execute_jurnal)>0) {
                                  while ($data_detil=mysqli_fetch_assoc($execute_detil)) {
                                    while ($data_jurnal=mysqli_fetch_assoc($execute_jurnal)) { ?>
                                    
                                    
                            <label for="tanggalT">Tanggal * : <i id="validtgl" style="color: red; font-weight: normal;"></i></label>
                            <input type="date" id="tanggalT" value="<?php echo $data_jurnal['tgl']; ?>" name="tanggalT" class="form-control" onchange="cekTGL(this.value);" required />

                            <label for="grouping">Grouping * :</label>
                            <select class="form-control" id="groupingT" name="groupingT" onchange="groupingbyr(this.value);" required>
                              <option><?php echo $data_detil['grouping']; ?></option>
                            ...
                            <textarea class="form-control" id="keteranganT" name="keteranganT" cols="100" rows="10" style="height: 60px; resize: none;" required><?php echo $data_detil['keterangan']; ?></textarea>

                            <label for="penerimaT">Penerima * :</label>
                            <input type="text" name="penerimaT" id="penerimaT" value="<?php echo $data_detil['penerima']; ?>" class="form-control" required>

                            <label for="cpenerimaT">CP Penerima * :</label>
                            <input type="text" name="cpenerimaT" id="cpenerimaT" value="<?php echo $data_detil['cp_penerima']; ?>" class="form-control">
                            ...
                            <select name="statusT" id="statusT" class="form-control" onchange="cekStatus(this.value);" onmo required>
                              <option><?php echo $data_detil['status']; ?></option>
                            </select>
 ...
 <script type="text/javascript">
  ...
  function cekStatus(statusT) {
    ...
    if (status=='BAYAR(TEPAT WAKTU)') {
      ...
      document.getElementById('buttonaksi').innerHTML='<button type="submit" class="btn btn-default"><i class="fa fa-edit"></i> Edit</button><button type="reset" class="btn btn-default"><i class="fa fa-refresh"></i> Reset</button>';
