...
if(!isset($_SESSION['username']) && !isset($_SESSION['password'])){ //cek apakah user sudah login
	...
	if ($_POST['statusT']=='BAYAR(TEPAT WAKTU)') {
		...
		$kodeD=$kode."BD";
		$kodeK=$kode."BK";
		$query_jurnalD="UPDATE jurnal SET tgl='$tanggal',kode_akun='1.1.1',nama_akun='Kas',debet='$nominal',kredit='0',ket='_' WHERE kode_p='$kodeD'";
		$query_jurnalK="UPDATE jurnal SET tgl='$tanggal',kode_akun='1.1.3',nama_akun='Piutang',debet='0',kredit='$nominal',ket='$KET' WHERE kode_p='$kodeK'";
		...
		$kodeD=$kode."BD";
		$kodeK=$kode."BK";
		$query_jurnalD="UPDATE jurnal SET tgl='$tanggal',kode_akun='1.1.1',nama_akun='Kas',debet='$bayar',kredit='0',ket='_' WHERE kode_p='$kodeD'";
		$query_jurnalK="UPDATE jurnal SET tgl='$tanggal',kode_akun='1.1.3',nama_akun='Piutang',debet='0',kredit='$bayar',ket='$KET' WHERE kode_p='$kodeK'";
		$execute_detail=mysqli_query($link, $query_detail);
		$execute_jurnalD=mysqli_query($link, $query_jurnalD);
		$execute_jurnalK=mysqli_query($link, $query_jurnalK);
		if (!$execute_detail && !$execute_jurnalD && !$execute_jurnalK) {
		...
		$status=$_POST['statusT'];
		$query_detail="UPDATE detail_piutang SET grouping='$grouping', keterangan='$keterangan', cp_penerima='$cppenerima',penerima='$penerima', bayar='0', sisa='$nominal', status='$status' WHERE kode_p='$kode'";
		$execute_detail=mysqli_query($link, $query_detail);
		if (!$execute_detail) {
				header('location: ../error_page/page_500.html');
			}else{
